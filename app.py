...
# Bot Token এবং Admin ID - আপনার টোকেন দিন
BOT_TOKEN = '8000160699:AAHq1VLvd05PFxFVibuErFx4E6Uf7y6F8HE'

# Bot Token validation
if not BOT_TOKEN or BOT_TOKEN == 'YOUR_BOT_TOKEN':
    raise ValueError("Please set your bot token in BOT_TOKEN variable")
...
def update_group_setting(group_id, setting, value):
    ...
    try:
        ...
        logger.error(f"Error updating group setting {setting} for {group_id}: {e}")
    finally:
        if conn:
            conn.close()
...
def get_admin_permissions(admin_id):
    ...
    try:
        ...
        logger.error(f"Error getting admin permissions for {admin_id}: {e}")
        return {}
    finally:
        if conn:
            conn.close()
...
def set_admin_permissions(admin_id, permissions):
    ...
    try:
        ...
        logger.error(f"Error setting admin permissions for {admin_id}: {e}")
    finally:
        if conn:
            conn.close()
...
def add_admin_to_group(admin_id, group_id, can_broadcast=0, can_manage_settings=0, can_ban_users=0, can_view_inbox=0):
    ...
    try:
        ...
        logger.error(f"Error adding admin {admin_id} to group {group_id}: {e}")
    finally:
        if conn:
            conn.close()
...
def get_admin_group_permissions(admin_id, group_id):
    ...
    try:
        ...
        logger.error(f"Error getting admin group permissions for {admin_id} in {group_id}: {e}")
        return None
    finally:
        if conn:
            conn.close()
...
def add_message_to_inbox(user_id, message_type, content, file_id=None):
    ...
    try:
        ...
        logger.error(f"Error adding message to inbox for user {user_id}: {e}")
    finally:
        if conn:
            conn.close()
        ...
        if chat_id in admin_panel_messages:
            for msg_id in admin_panel_messages[chat_id][:]:
                try:
                    ...
                    pass
            admin_panel_messages[chat_id] = [message_id] if message_id else []
        elif message_id:
...
def update_admin_panel_message(chat_id, message_id, text, reply_markup=None):
    ...
    try:
        ...
        try:
            ...
            new_msg = bot.send_message(chat_id, text, reply_markup=reply_markup, parse_mode='HTML', disable_web_page_preview=True)
            if chat_id in admin_panel_messages:
...
def get_admin_accessible_groups(admin_id):
    ...
    try:
        ...
        logger.error(f"Error getting accessible groups for admin {admin_id}: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
⏰ Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
        ...
        for admin_id, admin_username, _, _ in admins:
            ...
            try:
                ...
                logger.error(f"Error sending request to admin {admin_id}: {e}")
        
        if request_sent:
            bot.answer_callback_query(call.id, "✅ Your request has been sent to admins!")
        else:
            bot.answer_callback_query(call.id, "❌ No admins available. Please try again later.")
...
def handle_accept_request(call):
    ...
    try:
        ...
        if not session_id:
            bot.answer_callback_query(call.id, "❌ Failed to create session!", show_alert=True)
            return
...
def handle_reject_request(call):
    ...
    try:
        ...
        logger.error(f"Error in handle_reject_request: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
def handle_pause_session(call):
    ...
    try:
        ...
        logger.error(f"Error in handle_pause_session: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
def handle_resume_session(call):
    ...
    try:
        ...
        logger.error(f"Error in handle_resume_session: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
def handle_end_session(call):
    ...
    try:
        ...
        logger.error(f"Error in handle_end_session: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
{user_list}
        ...
        logger.error(f"Error in show_inbox_manager: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
No messages found in history.
            ...
            for i, msg in enumerate(reversed(messages), start=1):
                ...
                time_str = str(sent_at)[11:16] if len(str(sent_at)) > 16 else str(sent_at)
                
                if msg_type == 'text':
                    preview = content[:50] + "..." if content and len(content) > 50 else content or "[No text]"
                    ...
                    message_history += f"{i}. ❓ Unknown type [{time_str}]\n"
...
{message_history}
        ...
        logger.error(f"Error in view_user_messages: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
Use buttons below to manage the session.
        ...
        logger.error(f"Error in admin_start_chat: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
<b>Session Controls:</b>
        ...
        logger.error(f"Error in view_active_session: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
{group_list}
        ...
        logger.error(f"Error in show_group_manager: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
{group_list}
        ...
        logger.error(f"Error in show_my_groups: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
<b>Select action:</b>
        ...
        logger.error(f"Error in manage_group: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
def toggle_maintenance_mode(call):
    ...
    try:
        ...
        logger.error(f"Error in toggle_maintenance_mode: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
def toggle_link_filter(call):
    ...
    try:
        ...
        logger.error(f"Error in toggle_link_filter: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
def toggle_bot_status(call):
    ...
    try:
        ...
        logger.error(f"Error in toggle_bot_status: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
Send the message you want to broadcast to this group:
        ...
        logger.error(f"Error in group_broadcast_menu: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
3. <b>All Users</b> - Send to all registered users
        ...
        logger.error(f"Error in global_broadcast_menu: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
def leave_group_handler(call):
    ...
    try:
        ...
        logger.error(f"Error in leave_group_handler: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
{admin_list}
        ...
        logger.error(f"Error in show_admin_manager: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
2. Or send their username starting with @
        ...
        logger.error(f"Error in add_admin_menu: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
Send the admin's user ID to assign them to this group:
        ...
        logger.error(f"Error in assign_admin_to_group: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
Please send the new leave message:
        ...
        logger.error(f"Error in set_leave_message: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
Please send an image to use as warning:
        ...
        logger.error(f"Error in set_warning_image: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
def show_statistics(call):
    ...
    try:
        ...
        try:
            ...
            total_users = total_groups = total_admins = total_messages = 0
            active_sessions_count = active_today = messages_today = 0
        finally:
            if conn:
                conn.close()
...
def back_to_admin_panel(call):
    ...
    try:
        ...
        mock_msg = MockMessage(call.from_user.id, call.message.chat.id, call.from_user.first_name)
        handle_admin_panel(mock_msg)
...
def handle_private_message(message):
    ...
    try:
        ...
        if message.content_type == 'text':
            ...
            add_message_to_inbox(user_id, 'photo', message.caption, message.photo[-1].file_id if message.photo else None)
        elif message.content_type == 'video':
            add_message_to_inbox(user_id, 'video', message.caption, message.video.file_id if message.video else None)
        elif message.content_type == 'document':
            add_message_to_inbox(user_id, 'document', message.caption, message.document.file_id if message.document else None)
        elif message.content_type == 'sticker':
            add_message_to_inbox(user_id, 'sticker', None, message.sticker.file_id if message.sticker else None)
...
def handle_admin_id_input(message, state):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error getting chat info for {admin_id}: {e}")
            username = "Unknown"
            first_name = "Unknown"
...
Contact the super admin for additional permissions.
        ...
        # Clear state
        if message.from_user.id in user_message_states:
...
Select permissions to grant:
        ...
        logger.error(f"Error in handle_admin_group_assignment: {e}")
        bot.send_message(message.chat.id, "❌ An error occurred. Please try again.")
...
{new_message}
        ...
        logger.error(f"Error in handle_leave_message_input: {e}")
        bot.send_message(message.chat.id, "❌ An error occurred. Please try again.")
...
The new image will be used when deleting links.
        ...
        logger.error(f"Error in handle_warning_image_input: {e}")
        bot.send_message(message.chat.id, "❌ An error occurred. Please try again.")
...
Error: {str(e)}
        ...
        logger.error(f"Error in handle_group_broadcast_input: {e}")
        bot.send_message(message.chat.id, "❌ An error occurred. Please try again.")
...
def set_admin_permission(call):
    ...
    try:
        ...
        if updated_perms:
            ...
            perm_status = "No permissions set"
...
{perm_status}
        ...
        logger.error(f"Error in set_admin_permission: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
...
• 📥 Can view inbox for group
        ...
        logger.error(f"Error in save_all_permissions: {e}")
        bot.answer_callback_query(call.id, "❌ An error occurred. Please try again.")
    ...
    while True:
        try:
            ...
            for user_id in list(user_message_states.keys()):
                # Simple cleanup - you might want to add timestamp to states
                pass
...
def run_bot():
    ...
    while True:
        try:
            bot.remove_webhook()  # Remove webhook if exists
            time.sleep(1)
